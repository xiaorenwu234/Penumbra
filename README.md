...
演示六个场景：
- **Scenario 1 (Commit)**：agent 写文件 + IPC 冻结 → commit 提交文件并恢复进程
- **Scenario 2 (Rollback)**：agent 写文件 → IPC 冻结 → 回滚文件并杀死进程
- **Scenario 3 (Cascade)**：Agent-A 写入 → Agent-B 读取 → 回滚 A 级联到 B
- **Scenario 4 (COW Memory)**：进程修改全局变量 → COW 快照 → 回滚恢复原始内存
- **Scenario 5 (Exit Hold)**：agent 完成执行 → 透明挂起 → commit 释放进程
- **Scenario 6 (Priv Escalation)**：进程尝试 setuid(0) → eBPF 拦截 → 回滚
...
- **eBPF 拦截**：通过 LSM 钩子拦截网络、IPC、信号、ptrace、权限提升等系统调用
...
- **Exit Hold**：通过 LD_PRELOAD + BPF 哨兵地址检测拦截 `exit_group`，进程完成后透明挂起，等待 commit/rollback 决策
- **权限提升拦截**：通过 `bprm_check_security` 和 `task_fix_setuid` LSM 钩子拦截 setuid/setgid 执行和 UID 变更
